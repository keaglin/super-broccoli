...
querySelector & getElementById each return one elemnet
...
getElementByClass returns an array of nodes with that class name
getElementsByTagName allows you to select any html tag, will return an array of nodes with that tag

NOTE: nodelists are not actually arrays. they have indexes to access items but Array methods will not/do not work on them
